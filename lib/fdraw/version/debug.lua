...
local function checkForNonNill(value, name, pos)
    if value then
        return true
    end
    return false, name.."#"..pos.." can't be null!"
end

local function checkForNumber(value, name, pos)
    local result, msg = checkForNonNill(value, name, pos)
    if result then
        if type(value) == "number" then
            return true
        end
        return false, name.."#"..pos.." isn't a number!"
    end
    return false, msg
end

local function checkForPositiveAndZeroNumber(value, name, pos)
    local result, msg = checkForNumber(value, name, pos)
    if result then
        if value >= 0 then
            return true
        end
        return false,name.."#"..pos.."need's to be positive or zero!"
    end
    return false, msg
end

local function checkForPositiveNumber(value, name, pos)
    local result, msg = checkForNumber(value, name, pos)
    if result then
        if value > 0 then
            return true
        end
        return false, name.."#"..pos.."need's to be greater than zero!"
    end
    return false, msg
end
...
function op.new(width, height)
    try(checkForPositiveNumber(width, "Width", 1), error_level)
    try(checkForPositiveNumber(height, "Height", 2), error_level)
...
function op.select(index)
    try(checkForPositiveAndZeroNumber(index, "Index", "1"))
    selected_buff = index
end

function op.draw(f, ...)
    try(checkForNonNill(f, "Draw_func", 1), error_level)
    try(type(f) == "function", "Draw_func#1 need's to be a function!", error_level)
...
function op.fill(x, y, width, height, _char)
    try(checkForPositiveNumber(x, "X", 1), error_level)
    try(checkForPositiveNumber(y, "Y", 2), error_level)
    try(checkForPositiveNumber(width, "Width", 3), error_level)
    try(checkForPositiveNumber(height, "Height", 4), error_level)
    try(checkForNonNill(_char, "Char", 5))
...
function op.setf(color)
    try(checkForNumber(color, "Color", 1), error_level)
    try(color <= 0xffffff, "Color#1 out of range, range is 0 to 0xffffff", error_level)
...
function op.set(x, y, string)
    try(checkForPositiveNumber(x, "X", 1), error_level)
    try(checkForPositiveNumber(y, "Y", 2), error_level)
    try(checkForNonNill(string, "String", 3), error_level)
    try(type(string) == "string", "String#3 isn't a string!", error_level)

    local mem = vir_tree[selected_buff]
    for i=1, #string do
        char = string.byte(string, i)
        local pixel_tree = get_BackForeChar_pipeline(mem)
        local len = #pixel_tree
...
function op.setAll(array)
    try(checkForNonNill(array, "Array", 1), error_level)
    try(type(array) == "table", "Array#1 isn't a table!", error_level)
