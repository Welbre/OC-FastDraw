...
function op.bind(addr)
    local gpu = (addr == nil and require("component").getPrimary("gpu") or require("component").proxy(addr))
...
function op.fill(x, y, width, height, _char)
    ...
    local pixel_tree = get_BackForeChar_pipeline(vir_tree[selected_buff])
    for i = x, width do
        for j = y, height do
